...
export function pickSurpriseCity(answers: Pick<SessionAnswers, 'style' | 'group'>): string {
  const style = (answers.style || 'culture').toLowerCase();
  const group = (answers.group || 'solo').toLowerCase();
  const scored = SURPRISE_POOL.map(e => ({
    city: e.city,
    score: (e.styles.includes(style) ? 2 : 0) + (e.groups.includes(group) ? 1 : 0),
  }));
  const maxScore = Math.max(...scored.map(s => s.score));
  const best = scored.filter(s => s.score >= Math.max(maxScore - 1, 1));
  return best[Math.floor(Math.random() * best.length)].city;
}
...
export const api = {
  ...
  async generateTrip(answers: SessionAnswers): Promise<TripPlan> {
    ...
    // In 'suggest' mode, pick a personalized city using the shared picker
    ...
    if (isSurprise) {
      destination = pickSurpriseCity(answers);
      destMode = 'know';
      console.log(`[API] Surprise picked → ${destination}`);
    }
